...
analyticsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const campaignFilter = { campaign: { tenantId } };
    ...
    const [scansToday, scansWeek, scansMonth, activeCustomers, rewardsUnlocked, rewardsClaimed, totalCards, openAlerts] = await Promise.all([
      ...
      prisma.stampCard.count({ where: { campaign: { tenantId }, lastStampAt: { gte: monthAgo } } }),
      prisma.rewardRedemption.count({ where: campaignFilter }),
      prisma.rewardRedemption.count({ where: { ...campaignFilter, status: 'claimed' } }),
      prisma.stampCard.count({ where: { campaign: { tenantId } } }),
      prisma.fraudAlert.count({ where: { tenantId, status: 'open' } }),
    ]);

    // Repeat customers: cards with more than one granted stamp event.
    const cards = await prisma.stampCard.findMany({ where: { campaign: { tenantId } }, select: { stamps: true, cyclesDone: true, campaign: { select: { stampsRequired: true } }, _count: { select: { stampEvents: true } } } });
...
analyticsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const events = await prisma.stampEvent.findMany({
      where: { campaign: { tenantId }, status: 'granted', createdAt: { gte: start } },
...
analyticsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    const tenantId = tid(req);
    const events = await prisma.stampEvent.findMany({
      where: { campaign: { tenantId }, status: 'granted' },
...
analyticsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const campaigns = await prisma.campaign.findMany({
      where: { tenantId },
      include: { _count: { select: { stampEvents: true, redemptions: true, cards: true } } },
    });
    ok(res, campaigns
